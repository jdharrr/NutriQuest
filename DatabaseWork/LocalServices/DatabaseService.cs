    ...
    where TModel : IMongoDocument
{
    ...
    public async Task<BulkWriteResponse> BulkWriteAsync(List<WriteModel<TModel>> writeModels)
    {
        var result = await _collection.BulkWriteAsync(writeModels).ConfigureAwait(false);
        if (!result.IsAcknowledged)
            return new BulkWriteResponse { MatchedCount = 0, ModifiedCount = 0 };

        return new BulkWriteResponse { MatchedCount = result.MatchedCount, ModifiedCount = result.ModifiedCount };
    }
}